...
最慢那一批，而"一次超时只损失 2 篇"。正式 run 与 Discovery **共用同一个实现**
（`discovery.extract_places_from_evidences`），批大小/并发/归属规则不会两处漂移。
...
4. **缓存命中的并发竞态**：`search_terms` 里同名词在两个线程同时看到"未命中"就会
   真打两次高德（缓存的"查→未命中→调用"不是原子的）。除在节点层先去重外，
   `ProviderHub` 的计数器 / 缓存 / MCP 句柄创建也补上了锁（此前它已被 4 路并发使用）。

## 11. 仍然存在的瓶颈（按影响排序）

1. **Provider 长尾仍是最大项**：12306 与途牛火车两条线都出现过打满 90s 预算
...
2. **模型时延（critic + finalize ≈ 25~93s）**：本轮已经给了按用途的硬上限，
   但上限只能防"无限等"。5 天那次 92.8s 全部是模型抖动；要更快只能减少调用，
   而那会牺牲规划质量（critic 是质量门，不砍）。
