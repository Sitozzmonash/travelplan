...
| ④ 城市知识库 | `app/city_cache.py` 读写的 `city_pois` / `city_poi_mentions` / `city_evidences` / `city_cache_meta` | 目的地级（高德 POI + 攻略提及 + 攻略正文 + 检索词） | **跨会话、跨用户**，按城市分片 | 上述四张表 | 攻略 7 天 / POI 15 天（可配），过期先回旧值再后台刷新；写入时清理超期行 | `stage.status = CACHE`／`STALE_CACHE`、`prefetch.city_cache.*`、来源 `sources.status = CACHED` |
...
`prune_city_cache()` 只删各自 TTL 之外的行（攻略 7 天 / POI 15 天），所以"先用旧数据兜底、
再后台刷新"的语义不受影响。
...
它还会把"新鲜但没有社媒证据"（典型是 tikhub 限流）的城市标成**降级**并默认重跑。
...
把正文写进去 —— 也就是说**最多一个攻略 TTL（7 天）内自动补齐**，不需要手工迁移。
