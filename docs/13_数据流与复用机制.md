...
`discovery.resolve_pois(resolver, results, terms, extra_places)` 是 Discovery 与正式 run
**共用**的那一处收敛实现（"全收 + 按 id 去重"已被它取代）。**绝不空榜**，但兜底分两级：
...
| POI 详情 | 实体层带地址 + 营业时间的点已经**不再补查**（`PlaceResolver.needs_detail`，§2.2）；缺其中任一项的仍会补查一次（≤ `POI_DETAIL_LIMIT=6`） | 只按"两个字段都齐"才算够，宁可多查一条也不让行程缺营业时间 |
