...
[07_评测体系.md](07_评测体系.md) 第 6 节。当前全量 996 条，全部离线。
...
- 三步向导的后台取数（交通/酒店/攻略/地点）→ `app/discovery.py`（**唯一实现**，Session 与 Workflow 共用）；
...
- 策略 → 权重的映射 → `app/selection.py:transport_weights` / `hotel_weights` / `resolve_pace`。
  POI 的 MUST/WANT 已生效（`node_score_candidates`）。
