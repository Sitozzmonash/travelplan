...
| # | 代码事实 | 与 Vercel 的冲突 |
| --- | --- | --- |
| 1 | 一次真实规划**优化前基线实测 616 秒**（约 10.3 分钟；`_acceptance/perf_before.json`），优化后为 **7~9 分钟**（见第 6.2 节） | 函数最长执行时间：Hobby **300s**、Pro 默认 300s / 上限 **800s**。Hobby 必然 504 |
...
3. **预热城市知识库**（`scripts/preheat_cities.py`，见 [15_城市知识库攻略正文复用.md](15_城市知识库攻略正文复用.md) §9）：
