...
Run 结束（Agent 交卷 / 落库之前）
  ...
  → Evolution 只处理 analysis_status=pending 的记录（见 [EVOLUTION.md](EVOLUTION.md)）
  → 确认过的失败场景固化进 benchmark/cases/badcase_regression.jsonl（永久回归）
```

检测入口在两条路上是同一个：`app/workflow.py::_detect_and_save_badcases`（Agent 路径复用它），
由 `app/agent_runner.py::execute_agent_run` 在交卷后调用。检测失败或写库失败**绝不**把一次成功的
规划变成失败 run。
...
| `degradation` | medium | 本次 run 有任何降级说明（模型/Provider 不可用、循环被超时/步数/token 截断、时间冲突未解决），整体记一条 |
...
### Jev（7 类，**只在旧固定流程的 run 上可能出现**）

主规划已切到 Agent Loop，主路径没有 Jev 调用，因此这七类在 Agent 路径上恒不触发；
规则本身仍保留（旧流程与历史 run 复核时用）。
