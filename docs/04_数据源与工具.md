...
| `app/workflow.py:_record_provider_calls`（`node_finalize`） | `run`（`benchmark` 分支存在但**当前不可达**） | 正式 run |
...
`source_type` 区分 Discovery 与正式 Run，因此能回答"某个 Provider 在正式 Run 正常、但
Discovery 阶段大量超时"这类问题。
